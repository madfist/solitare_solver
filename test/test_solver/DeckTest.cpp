...
TEST(deck_test, deal) {
    ...
    EXPECT_EQ(Card("DQ_"), pile2.front());
    EXPECT_EQ(Card("D8_"), pile2.back());
    Pile pile3 = deck.deal(1, 1);
    EXPECT_EQ(2, pile3.size());
    EXPECT_EQ(Card("D7_"), pile3.front());
    EXPECT_EQ(Card("D6^"), pile3.back());
}
...
TEST(pile_test, io) {
    ...
    in >> pile;
    EXPECT_THAT(pile, testing::ElementsAre(Card("CA^"), Card("HK_"), Card("D3_")));
